...
async function readShamirWallet() {
  ...
  try {
    const currentState = fs.readFileSync("./json/ShamirEventWallet.json");
    if (currentState) {
...
async function detailedList(attendeeIndex) {
  ...
  for (let i = 0; i < aList.count; i++) {
    ...
    if (idList.indexOf(aTokenJson.attendeeID) === -1) {
      ...
      console.log(
        `${i + 1} : ${aList.messageIds[i]} \n\t ${aTokenJson.attendeeID} - ${aTokenJson.remark
...
async function closeEvent(attendeeIndex) {
  ...
  for (let i = 0; i < aList.count; i++) {
    ...
    if (attList.indexOf(aTokenJson.attendeeID) === -1 && usedShares.indexOf(aTokenJson.attendeeShare) == -1) {
      attList.push(aTokenJson.attendeeID);
      usedShares.push(aTokenJson.attendeeShare);
    }
  }
